 ...
 */
let num = 0;
const createTag = (elementA, elementH, elementL) => {
    elementA.href = `#${elementH.textContent}${num}`;
    elementA.textContent = elementH.textContent;
    if (elementH.textContent)
        elementH.id = elementH.textContent + num;
    elementL.append(elementA);
    num++;
};
const generateToc = (titles) => {
    const ul = document.createElement('ul');
    ul.className = 'main-toc';
    let h2u = document.createElement('ul');
    let h3u = document.createElement('ul');
